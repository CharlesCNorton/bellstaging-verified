...
(*                              CURE LIST                                     *)
(*                                                                            *)
(*  Cured: #1 ChangeRate/rate_to_nat, #2 calcium_replacement_needed,         *)
(*         #3 shock_index_elevated, #4 hyperglycemia/hypoglycemia/           *)
(*         respiratory_acidosis.                                              *)
(*                                                                            *)
(*  1. Wire days_to_full_feeds into FeedingProtocol: prove computed          *)
(*     refeeding duration is bounded by npo_duration_by_stage.               *)
(*  2. Wire Microbiology culture timing fields (collected_h,                  *)
(*     resulted_h) into antibiotic escalation decisions: no positive         *)
(*     culture after threshold hours gates coverage broadening.              *)
(*  3. Instantiate NeuroStatus in ClinicalState and wire                      *)
(*     neurologic_score into overall_severity_score.                         *)
(*  4. Replace tautological lemmas (classification_complete_and_unique,       *)
(*     every_patient_staged, breast_milk_preferred) with non-trivial         *)
(*     properties: classification stability under sign-preserving            *)
(*     state changes, breast milk reduces computed risk score.               *)
...
(*  7. Add invariant linking ClinicalState.hours_since_symptom_onset          *)
(*     to TimeSeries.obs_time_hours (consistency between record and          *)
(*     containing observation).                                              *)
(*  8. Prove compute_systemic_level agrees with classify_stage's              *)
(*     effective_stage3_sys / effective_stage2b_sys computations.            *)
(*  9. Enforce assessment staleness: require signs_current = true as          *)
(*     precondition on classifier output.                                    *)
(*  10. Characterize exact disagreement states between                        *)
(*      classify_declarative and classify_stage.                             *)
(*  11. Mitigate integer-division truncation in compute_trajectory            *)
(*      velocity near threshold with rounding-aware comparison.              *)
(*  12. Replace severity_score_max exhaustive 2^10 destruction with           *)
(*      scalable proof strategy (bounded sum over weighted booleans).        *)
(*  13. Prove classify_inputs faithfully mirrors classify_stage:              *)
(*      bridge ClassifierInputs monotonicity to ClinicalState.t so          *)
(*      classify_inputs_monotone lifts to the concrete classifier.          *)
(*  14. Route Microbiology culture results into Antibiotics module            *)
(*      for culture-directed therapy selection.                              *)
(*  15. Prove FeedingProtocol.can_restart_feeds returns false during           *)
(*      active NEC (classification >= IIA): refeeding safety invariant.       *)
(*  16. Wire NeonatalOrganFailure scores into classifier as severity          *)
(*      modifier for Stage III substaging.                                   *)
(*  17. Calibrate DifferentialDiagnosis confidence weights against             *)
(*      published sensitivity/specificity data.                              *)
(*  18. Refine SurgicalProcedures.initial_procedure_for_perforation           *)
(*      to account for NET trial nuance (Moss et al. 2006).                 *)
(*  19. Replace Prognosis nat with record { low; mid; high } to               *)
(*      reflect published ranges rather than point estimates.                *)
(*  20. Guard RiskFactors.risk_score against nat underflow: replace           *)
(*      nat subtraction with Z arithmetic.                                   *)
(*  21. Add module with published case studies (Sharma & Hudak 2013,          *)
(*      Neu & Walker 2011) as witness validation suite.                      *)
(*  22. Prove suggests_volvulus and suggests_sepsis_without_nec are            *)
