...
(*                              CURE LIST                                     *)
(*                                                                            *)
(*  1. Delete unused ChangeRate type and rate_to_nat from                     *)
(*     TemporalProgression.                                                   *)
(*  2. Delete unused calcium_replacement_needed from CoagulationPanel.        *)
(*  3. Delete unused shock_index_elevated from VitalSigns.                    *)
(*  4. Delete unused hyperglycemia, hypoglycemia, respiratory_acidosis        *)
(*     from LabValues.                                                        *)
(*  5. Delete unused days_to_full_feeds from FeedingProtocol.                 *)
(*  6. Delete unused Microbiology culture timing fields (collected_h,         *)
(*     resulted_h) or wire them into decision logic.                          *)
(*  7. Delete unused NeuroStatus/neurologic_score or instantiate              *)
(*     somewhere in the codebase.                                             *)
(*  8. Delete or replace tautological lemmas (classification_complete-        *)
(*     _and_unique, every_patient_staged, breast_milk_preferred) with         *)
(*     proofs that assert non-trivial properties.                             *)
(*  9. Define valid_transition as inductive relation with eauto hints.        *)
(*  10. Add Stabilization -> SurgicalEvaluation to valid transitions.         *)
(*  11. Add invariant linking ClinicalState.hours_since_symptom_onset         *)
(*      to TimeSeries.obs_time_hours (consistency between record and          *)
(*      containing observation).                                              *)
(*  12. Prove compute_systemic_level agrees with classify_stage's             *)
(*      effective_stage3_sys / effective_stage2b_sys computations.            *)
(*  13. Enforce assessment staleness: wrap classifier output in option         *)
(*      or require signs_current = true as precondition.                      *)
(*  14. Prove classify_declarative and classify_stage never diverge by         *)
(*      more than 1 stage, or characterize exact disagreement states.         *)
(*  15. Document or mitigate integer-division truncation edge cases in         *)
(*      compute_trajectory velocity calculation near threshold.               *)
(*  16. Replace severity_score_max exhaustive 2^10 destruction with           *)
(*      scalable proof strategy.                                              *)
(*  17. Prove that classify_inputs faithfully mirrors classify_stage:          *)
(*      bridge ClassifierInputs monotonicity to ClinicalState.t so           *)
(*      classify_inputs_monotone lifts to the concrete classifier.           *)
(*  18. Route Microbiology culture results into Antibiotics module for         *)
(*      culture-directed therapy selection.                                   *)
(*  19. Prove FeedingProtocol.can_restart_feeds returns false during           *)
(*      active NEC (classification >= IIA): refeeding safety invariant.       *)
(*  20. Wire NeonatalOrganFailure scores into classifier or document          *)
(*      why organ failure assessment is intentionally disconnected.           *)
(*  21. Calibrate DifferentialDiagnosis confidence weights against             *)
(*      published sensitivity/specificity data.                               *)
(*  22. Refine SurgicalProcedures.initial_procedure_for_perforation           *)
(*      to account for NET trial nuance (Moss et al. 2006).                  *)
(*  23. Replace Prognosis nat with record { low; mid; high } to               *)
(*      reflect published ranges rather than point estimates.                 *)
(*  24. Guard RiskFactors.risk_score against nat underflow (protective         *)
(*      > raw clips to 0 silently; document or use Z).                       *)
(*  25. Add module with published case studies (Sharma & Hudak 2013,          *)
(*      Neu & Walker 2011) as witness validation suite.                       *)
(*  26. Prove suggests_volvulus and suggests_sepsis_without_nec are            *)
(*      exercised beyond most_likely_diagnosis, or delete.                    *)
(*                                                                            *)
(******************************************************************************)
